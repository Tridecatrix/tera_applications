...
echoheader() {
    echo "iostat  failed_reads failed_writes invalid_io notify_free"
    echo "mmstat  orig_data_size compr_data_size mem_used_total mem_limit mem_used_max same_pages pages_compacted huge_pages huge_pages_since"
    echo "bdstat  bd_count bd_reads bd_writes"
    echo
}
...
echo 0 | sudo tee /sys/fs/${ZRAMDEV}/mem_used_max > /dev/null
